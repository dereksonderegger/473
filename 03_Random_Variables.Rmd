...
$$ g_1(x|\pi) = {n \choose x} \pi^x (1-\pi)^{n-x} \;\cdot I(x\in \{0,1,\dots,n\})$$
For this problem, we have $n=40$ and $\alpha=\beta=2$ and we are interested in random variables $X$ and $\Pi$.
    ...
    b) Find the marginal distribution of $X$. _Hint: Distributions must integrate/sum to one, and therefore distributions lists are a convenient set of integration/sumation identities. In particular from our definition of the Beta Distribution we have:_
...
Further suppose that $Y=X(1-X)$. Find the pdf of $Y$. 
    ...
    c) On your graph of $y=x(1-x)$, pick a value for $y$ and find the region(s) of $x$ such that $x(1-x)\le y$. _Hint: quadratic equation!_
...
$$f(x) = e^{-x} \; \cdot I(0 < x)$$
Further suppose that $Y=\Big\vert\sqrt{X}\Big\vert$. Find the pdf of $Y$.
...
**CDF Method**
Our most widely applicable method for dealing with transformations is to consider how the tranformation can affect the CDF.
...
$$\begin{aligned} G(Y) 
  ...
  = Pr( X_1 + X_2 \le y ) 
  \end{aligned}$$
```{r, cache=TRUE, echo=FALSE, warning=FALSE, message=FALSE, dev='jpeg'}
library(dplyr); library(ggplot2)
data <- expand.grid(x1 = seq(0, 1.5, by = 0.001), 
                    x2 = seq(0, 1.5, by = 0.001)) %>%
  mutate( density = dexp(x1) * dexp(x2) ) %>%
  filter( x1 + x2 < 1 )
ggplot( data, aes(x=x1, y=x2, fill=density)) + 
  geom_tile() + 
  scale_x_continuous(breaks = c(0, 1), labels = c(0, 'y'), name=expression(x[1])) +
  scale_y_continuous(breaks = c(0, 1), labels = c(0, 'y'), name=expression(x[2])) +
  scale_fill_continuous(low='blue', high='red', name=expression(f( x[1], x[2]) ) )
```
...
$$\begin{aligned} g(y) = \frac{d}{dy} G(y) 
...
**Convolutions** 
We are often interested in the sum of two independent random variables. Suppose that $X_1$ has density function $f_1(x_1)$ and $X_2$ has density function $f_2(x_2)$ and $X_1$ is independent of $X_2$. Define $Y=X_1 + X_2$. We say that the distribution of $Y$ is the _convolution_ of the distributions of $X_1$ and $X_2$. 
...
$$g(y) = \int_0^y 1 \, du = y \;\;\;\;\; \textrm{ if } y \in (0,1)$$
and in the second case
$$g(y) = \int_{y-1}^1 1 \, du = 2-y \;\;\;\;\; \textrm{ if } y \in (1,2)$$
so all together
$$g(y) = \begin{cases}
  0   & \textrm{ if }\; y \le 0       \\
  y   & \textrm{ if }\; 0 < y \le 1   \\
...
library(dplyr)
library(ggplot2)
data <- data.frame( x=seq(-1,3, by=.1) ) %>%
  mutate(density = 0 * (x<0) + x * (0<=x & x<1) + (2-x) * (1 <= x & x < 2) + 0*(2<=x) )
ggplot(data, aes(x=x, y=density)) + geom_line() + labs(y='g(y)', x='y')
```
...
_Hint: Don't ignore the indicator functions._


4. **Distribution of the Maximum of a sample.** Suppose that $X_1, X_2, \dots, X_n$ are independent random variables, and all have a common distribution with CDF $F(x)$ and pdf $f(x)$. Let $Y_n$ be the maximum value of these $X_1, X_2, \dots, X_n$ values.  _For convenience, let $X_i$ be the $i$th of these._
    a) If for some $y$ we have that $Y_n \le y$, what inequality must hold for each $X_i$?
    b) Write $Pr( Y_n \le y )$ as some probability statement involving all $X_1, X_2, \dots, X_n$ independent variables.
    c) Write down the formula for $G_n(y)$.
    d) Write down the formula for $g_n(y)$.
    
5. **Distribution of the Minimum of a sample.** Suppose that $X_1, X_2, \dots, X_n$ are independent continuous random variables, and all have a common distribution with CDF $F(x)$ and pdf $f(x)$. Let $Y_1$ be the minimum value of these $X_i$ values.
    a) If for some $y$ we have that $Y_1 > y$, what inequality must hold for each $X_i$?
    b) Write $1 - Pr( Y_1 > y )$ as some probability statement involving all $X_1, X_2, \dots, X_n$ independent variables.
...
$$f_1(x_1) = \frac{1}{b-a} \;\cdot I(a \le x_1 \le b)$$
$$f_2(x_2) = \frac{1}{b-a} \;\cdot I(a \le x_2 \le b)$$
...
**pdf method**
Just as in the univariate case, we might ask if there is a way to just go straight from the joint pdf to the pdf of the variable of interest.  The answer is yes provided, we convert the distribution of $X_1,X_2,\dots,X_n$ into $Y_1,Y_2,\dots,Y_n$ via transformations $r_1(),r_2(), \dots,r_n()$ where
...
$$x_2 = s_2(y_1, y_2, \dots, y_n)$$
$$\vdots$$
$$x_n = s_n(y_1, y_2, \dots, y_n)$$

For notational compactness, denote $s_1 = s_1(y_1, y_2, \dots, y_n)$. Then
$$g(y_1, y_2, \dots, y_n) = f(s_1, s_2, \dots, s_n) \cdot \big\vert J \big\vert$$
...
In this case $X_1$ and $X_2$ are positive values, so $Y_1 \in (0,1)$ and $Y_2 > 0$.
Doing the backsolving for $X_1$ and $X_2$ we have
$$x_1 = y_1 y_2$$
$$x_2 = y_2 ( 1-y_1 )$$
and therefore
...
y_2 & y_1 \\
-y_2 & 1-y_1
\end{array}\right] = y_2(1-y_1) +y_1 y_2 = y_2$$

Therefore
$$\begin{aligned} g(y_1, y_2) 
...
$$f(x_1, x_2) = 2(1-x_1)\; I(0\le x_1 \le 1) I(0\le x_2 \le 1)$$
Find the pdf of $Y_2=X_1X_2$. _Because the pdf method of transformation requires having a $Y_1$ variable, we will consider a second random variable that is convenient, $Y_1 = X_1$._

9. Suppose that $X_1$ and $X_2$ have a continuous joint distribution with pdf
$$f(x_1, x_2) = 8 x_1 x_2 \,\cdot I(0 < x_1< x_2 < 1)$$

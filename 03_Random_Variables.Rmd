...
1. Suppose that $X_1$ and $X_2$ have support along the entire real line. Show that the density function of $Y$ is
$$g(y) = \int_{-\infty} ^{\infty} f_1(y-u)f_x(u) \, du = \int_{-\infty} ^{\infty} f_1(z)f_x(y-z) \, dz$$
    ...
    c) Recognize that in the set up of $G(y)$ we have indirectly figured out what $g(y)$.
...
4. **Distribution of the Maximum of a sample.** Suppose that $X_1, X_2, \dots, X_n$ are independent random variables, and all have a common distribution with CDF $F(x)$ and pdf $f(x)$. Let $Y_n$ be the maximum value of these $X_i$ values.
    a) If for some $y$ we have that $Y_n \le y$, what inequality must hold for each $X_i$?
    b) Write $Pr( Y \le y )$ as some probability statement involving all $X_1, X_2, \dots, X_n$ independent variables.
...
where $\beta > 0$. Show that the pdf of $Y=X_1 + X_2$ is
$$g(y) = \beta^2 y e^{-\beta y} \; \cdot I(y>0)$$
Find the pdf of the minimum of $X_1$ and $X_2$.

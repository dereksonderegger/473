...
```{r, cache=TRUE, message=FALSE, warning=FALSE, echo=FALSE, fig.height=3}
library(ggplot2); library(dplyr);
...
We might now ask question such as "What is the probability that $X<.5$?" or "What is the probability that $Y>X^2$?  In both cases, we just need to integrate the density across the full area of interest.

...
7. *Binomial Expectation* Suppose that we have $n$ indentically distributed Bernoulli random variables, each of which having probability of success $f_i(1)=p$. Letting $Y=\sum_1^n X_i$, what is the expected value of $Y$?
...
1. Suppose that the random variable $X$ has an $Exponential(\beta)$ distribution which is a special case of the Gamma distribution. $Exponential(\beta) = Gamma(1,\beta)$ distribution. The table of distributions in your book shows that the $Var(X)=\beta^2$. Derive the mgf of the Exponential distribution and use it to derive both the expectation and variance.
...
9. Suppose that $X_1, X_2, \dots, X_n$ are indepenent random variables each with distribution $Poisson(\lambda)$. Derive the moment generating function of the distribution of $Y=\sum X_i$ and state its distribution.

10. Suppose that $X_1, X_2, \dots, X_n$ are indepenent random variables each with distribution $Exponential(\beta)=Gamma(1,\beta)$. Derive the moment generating function of the distribution of $Y=\sum X_i$ and state its distribution.
...
5. Show that the preceeding statement is not an "if and only if" statement by considering the following case.  Let be uniformly $X$ distributed on the integers $-1, 0,$ and $1$. Let $Y=X^2$. Show that $X$ and $Y$ are not independent but that $Cov(X,Y)=0$.
...
_Example_ Recall the Beta-Binomial hierarcical relationship where we have some observation $X$ that is the number of successes out of $n$ independent Bernoulli$(P)$ trials, and the probability of success $P$ was also a random variable but with a Beta($\alpha$,$\beta$) distribution.
$$X \sim Binomial(n,P) \;\;\;\;\;\;\textrm{where }\;\;\; P \sim Beta(\alpha, \beta)$$
...
If we regard $P=p=0$ as a known quantity, then 
$$E(X\,|\,P=0.6) = np = n(0.60)$$
...
2. Suppose that $X\sim Binomial(n,P)$ and $P\sim Beta(\alpha=4,\beta=6)$. Find $E(X)$. _Hint: Though we haven't proven it (yet!), the expectation of a Beta random variable is $\alpha/(\alpha + \beta)$_. 

3. Suppose that an unknown number of individuals (denoted as $N$) will be independently randomly choosen for "additional screening" by the TSA. Suppose that we allow $N\sim Poisson(\lambda)$ individuals are selected and that the probability that a selected person is female is $0.4$. Let $X$ denote the number of females (out of $N$ individuals additionally screened) and so $X \sim Binomial(N, p=0.5)$. Find $E(X)$. _Hint: Though we haven't proven it yet, the variance of a Poisson random variable is also $\lambda$._
...
4. Suppose that an unknown number of individuals (denoted as $N$) will be independently randomly choosen for "additional screening" by the TSA. Suppose that we allow $N\sim Poisson(\lambda)$ individuals are selected and that the probability that a selected person is female is $0.4$. Let $X$ denote the number of females (out of $N$ individuals additionally screened) and so $X \sim Binomial(N, p=0.5)$. Find $Var(X)$. 
